...
const TableUi = ({
  ...
  async function getSubcategory(tablePagination, PageSize) {
    setLoading(true);
    const variables = {
  ...
  useEffect(() => {
    getSubcategory(tablePagination, pageSize);
  ...
  const columns = [
    ...
    {
      title: 'Subcategory Titles Associated',
      dataIndex: 'subcategory_title',
      key: 'subcategory_title',
      align: 'center',
      render: (_, {subcategory_title}) => (
        <>
          {subcategory_title?.map((name) => {
