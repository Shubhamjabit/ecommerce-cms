...
const TableUi = ({
    ...
    setLoading(true);
    {
      ...
      try {
        const data = await axios.post(
          `${envUrl.baseUrl}${cmsendPoint.getMainBannerTable}`,
          variables,
          {
            headers: {
        ...
        setBannerData(data.data.data.mainbannerData.data);
        setLoading(false);
  ...
  useEffect(() => {
    if (editbannerflag || addbannerflag || deletebannerflag) {
      getBanner(tablePagination, PageSize);
    }
  }, [editbannerflag, addbannerflag, deletebannerflag]);
