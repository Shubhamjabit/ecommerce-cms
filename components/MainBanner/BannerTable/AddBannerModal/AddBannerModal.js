...
const AddBannerModal = ({visible, handleOk, handleCancel, AddBannerflag}) => {
  ...
  */
  ...
  function onChange(value, dateString) {
    ...
    setFromdate(
      // moment(dateString[0], 'DD-MM-YYYY HH:mm:ss').format('YYYY-MM-DD HH:mm:ss')
      dayjs(dateString[0], 'DD-MM-YYYY HH:mm:ss').format('YYYY-MM-DD HH:mm:ss')
    );
    setTodate(
      // moment(dateString[1], 'DD-MM-YYYY HH:mm:ss').format('YYYY-MM-DD HH:mm:ss')
      dayjs(dateString[1], 'DD-MM-YYYY HH:mm:ss').format('YYYY-MM-DD HH:mm:ss')
    );
    //setTodate(dateString[1]);
  }

  const currentfromdate = dayjs()
    .tz('Australia/Sydney')
    .format('YYYY-MM-DD HH:mm:ss');

  const currenttodate = dayjs()
    .tz('Australia/Sydney')
    .add(2, 'M')
    .format('YYYY-MM-DD HH:mm:ss');
  useEffect(() => {
    setFromdate(dayjs(currentfromdate).format('YYYY-MM-DD HH:mm:ss'));
    setTodate(dayjs(currenttodate).format('YYYY-MM-DD HH:mm:ss'));
  }, []);
        ...
        >
          ...
          <Form.Item label="Schedule Date" name="date">
            <RangePicker
              ...
              defaultValue={[dayjs(currentfromdate), dayjs(currenttodate)]}
              onChange={onChange}
              onOk={onOk}
