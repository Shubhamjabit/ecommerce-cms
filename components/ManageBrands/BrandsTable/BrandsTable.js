...
const TableUi = ({
  ...
  const getCategory = async (tablePagination, PageSize) => {
    setLoading(true);
    {
      ...
      try {
        await axios
          .post(`${envUrl.baseUrl}${cmsendPoint.getBrandsTable}`, variables, {
            headers: {
          ...
          .then((res) => {
            ...
            if (res.status == 200) {
              setCategory(
                ...
                res.data.data.BrandsTablecms.data
              );
              setCategoryTotal(res.data.data.BrandsTablecms.total);
              setLoading(false);
        ...
        console.log('error getBrandsTable:', error.message);
