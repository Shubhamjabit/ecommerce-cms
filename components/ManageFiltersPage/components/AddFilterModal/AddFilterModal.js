...
const AddFilterModal = ({
  ...
  // console.log('render add filter');
  const reactTagRef = useRef(null);
  ...
  const handleAddition = (tag) => {
    setTags([...tags, tag]);
    // to remove validation error on addition
  ...
  const handleInputChangeFunc = (value) => {
    setFilterError(false);

    console.log('ccccccc', reactTagRef);
    // reactTagRef.current.value = filterValue;
    // if (filterError) {
    //   setTimeout(() => {
    //     console.log('vvvvv', value);
    //     document.getElementsByClassName('ReactTags__tagInputField').value =
    //       value;
    //   }, 1000);
    // }
    setFilterValue(value);
    form.setFieldValue('filters', value);
  };
  // useEffect(() => {
  //   if (filterError) {
  //     setFilterError(false);
  ...
  // }, [filterValue]);
  const handleFocus = () => {
    // This function will be called when the input gains focus
