...
const EditFilterModal = ({
  ...
  let data = useSelector((state) => state.manageFilterReducer.data);
  // data = data.data;
  ...
  useEffect(() => {
    if (data) {
      ...
      var object = data.filterJson.reduce(
        (obj, item) => Object.assign(obj, {[item.id]: item.filterName}),
        {}
      );
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const data = await axios
        .post(
          `${envUrl.baseUrl}${cmsendPoint.updateManageFilter}`,
          jsonToPost,
          {
            headers: {
        ...
        .then(async function (response) {
          ...
          if ((response.data.message = 'success')) {
            ...
            await setRefreshState(refreshState + 1);
  ...
  const deleteFilter = (id) => {
    axios
      .delete(`${envUrl.baseUrl}${cmsendPoint.deleteFilter}`, {
        data: {id: id},
      })
      .then((res) => {
        if (res.status == 200) {
              ...
              <Row>
                <Col span={22}>
                  <Form.Item
                    ...
                    name={d.id}
                    rules={[
                      {required: true, message: 'Please enter Filter name'},
                    ]}
                ...
                <Col span={1} offset={1}>
                  {d.isLinked > 0 ? (
                    <Tooltip
                      placement="right"
                      title={
                        'Filter cannot be deleted since it is linked with a Sub Category'
                      }
                      trigger={'hover'}
                      color={'red'}
                    >
