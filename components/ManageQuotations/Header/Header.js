...
import Excelexport from '../ManageQuotationsTable/components/Excelexport';
...
const Header = ({
  ...
  console.log('originalListtttttttttttttttttttttttttttttt', originalList);
  ...
  const handleSearchTextChange = (e) => {
    ...
    updatedList = updatedList.filter((item) => {
      return (
        item.quoteId.toLowerCase().indexOf(query.toLowerCase()) !== -1
        // ||
        // item.erpNumber.toLowerCase().indexOf(query.toLowerCase()) !== -1
      );
    });
  ...
  return (
    <>
      <Row className={stlyes.mainRow} id="header-row">
        {isOrdersList ? (
          <Col span={5}>
            <p className={stlyes.topLittle}>Manage Quotations</p>
          </Col>
        ) : (
          <Col span={17}>
            <p className={stlyes.topLittle}>Manage Quotations</p>
