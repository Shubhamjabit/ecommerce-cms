...
const TableUi = ({
  ...
  const [banner, setBannerSecond] = useState(null);
  ...
  useEffect(() => {
    ...
    getBannerSecond(tablePagination,
        tPageSize
     );
  }, [tablePagination]);
  ...
  // useEffect(() => {
  //   if (data) {
  //     setBannerSecond(data.homebannersecond);
  //   }
  // }, [data]);
    ...
    setLoading(true);
      ...
      try {
        const data = await axios.post(
          `${envUrl.baseUrl}${cmsendPoint.getSecondBanner}`,
          variables,
          {
            headers: {
        ...
        // setData(data);
        setBannerSecond(data.data.data[0].homebannersecond);
        setLoading(false);
