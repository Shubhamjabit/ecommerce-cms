...
const PublishPopup = ({visible, handleOk, handleCancel, handleActive}) => {
  ...
  const active_user = async () => {
    ...
    try {
      ...
      const data = await axios
        .post(`${envUrl.baseUrl}${cmsendPoint.activeCmsUser}`, variables, {
          headers: {
        ...
        .then(function (result) {
          if (result) {
            if (result.data.activeUserStatus === 'success') {
      ...
      >
        <Row>
          <Col span={1} offset={11}>
            <ExclamationCircleOutlined className={styles.popupBoxIcon} />
          </Col>
        </Row>
        <Row>
          <Col span={16} offset={4}>
            <dev className={styles.textArea}>
              <p className={styles.text1}>Active the User</p>
