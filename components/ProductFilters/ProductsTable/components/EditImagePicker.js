...
const EditImagePicker = ({
  ...
  console.log('imagre preview @@@@ -1', images);
  ...
  useEffect(() => {
    if (images) {
      ...
      setFileList(imgs);
      dispatch(setProductMediaList(imgs));
    }
  }, []);
  ...
  const removeFileToBlob = async (file, index, fileType) => {
    ...
    await axios
      ...
      .then((response) => {
        ...
        if (response.status == 200) {
          console.log("file.name.split('/')[1]", file.name.split('/')[1]);
          if (file.name.split('/')[1] == defaultImageValue) {
            form.setFieldsValue({
...
export default EditImagePicker;
